...
if(isset($_GET['f']) && isset($_GET['n'])){
	...
	if(isset($_GET['d'])){
		$directory=SQL::decrypt_k($_GET['d']).'/';
	}else{
		$directory="";
	}
	$path=DATA_DIR.$directory.$file_name;
	$content_type=mime_content_type($path);
	$length=filesize($path);
	header('Content-Type: '.$content_type);    //コンテントタイプ
	header('Content-Disposition: inline; filename="'.$filename.'"');   //DLの場合は、Content-Disposition: attachment;表示はinline
