...
if(isset($_GET['i'])){
	...
	if($id!==0 && $data=SQL::fetch("SELECT student_user_id,title,title_iv,content,content_iv,edit_date,read_condition,reply FROM {$school_id}_manage_user_info WHERE chat_id=? ORDER BY edit_date",array($id))){
		$title="個別連絡詳細";
		$secret_text='manage_user_info_view';
		SQL::execute("UPDATE {$school_id}_manage_user_info SET read_condition=1 WHERE chat_id=? AND read_condition=0 AND student_user_id=0 AND teacher_user_id=?",array($id,$user_id));
		if(isset($_POST['btn']) && SQL::decrypt_ks($_POST['btn'])===$secret_text && isset($_POST['token']) && ALL::csrf_check($_POST['token'],$secret_text)){
			if(isset($_POST['message'])){
				...
				$values[]=$user_id;
				$values[]='';
				$row=SQL::fetch("SELECT student_user_id FROM {$school_id}_manage_user_info WHERE chat_id=? AND student_user_id!=0 ORDER BY edit_date",array($id));
				$values[]=$row['student_user_id'];
				$values[]='';
				$values[]='';
				...
				if(SQL::execute("INSERT INTO {$school_id}_manage_user_info VALUES (?,?,?,?,?,?,?,?,?,0,'',NOW())",$values)){
					$message="<h2>個別連絡のお知らせ</h2>".USER::name($data['student_user_id'])."&nbsp;様<p>".USER::name($user_id)."&nbsp;様から個別連絡への返信が届いております。<br><a href=".'"'.$url.'">LiCSS</a>にログインしてから個別連絡を確認してください。';
					ALL::send_html_mail(USER::email($data['student_user_id']),"個別連絡のお知らせ",$message);
