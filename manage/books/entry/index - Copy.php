	...
	<form method="post">
		<div id="manage_books_entry_wrapper">
			<div class="manage_books_entry_book_info">
				...
				<div>
					<input type="number" name="isbn" id="manage_books_entry_isbn" placeholder="ISBN" title="ISBNを入力してください" value="<?=$isbn;?>" onChange="input_isbn(this.value);" autofocus required>
					<button type="button" onClick="input_isbn(document.getElementById('manage_books_entry_isbn').value);">
						ISBNから書誌情報を取得
					</button>
					※入力内容は破棄されます。
				</div>
		...
		?>
				...
				<div>
					<select name="ndc" title="NDCを選択してください" id="manage_books_entry_ndc" required>
						<option label="選択してください"></option><?php
		...
		foreach($stmt as $row){
			...
			?>
						<option value="<?=SQL::encrypt_ks($row['ndc_id']);?>" title="<?=ALL::h($row['ndc_code']);?>&nbsp;<?=$name;?>"<?=$select;?>>
		...
		<script>
			...
			function input_isbn(isbn){
				...
				manage_books_entry_get_book_info(isbn);
			}
