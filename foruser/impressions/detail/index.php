...
<nav id="bread_crumb">
	<ul id="bread_crumb_list">
		...
		<li>
			<a href="<?=$url;?>foruser/impressions/">
				感想文一覧
			</a>
		</li>
		<li>
...
<section id="main_content"><?php
	...
	<form method="post">
		...
		if($data['public']==0){
			?>
						<textarea name="content" placeholder="感想文" title="感想文を入力してください" required><?=$data['content'];?></textarea><?php
		}else{
			echo ALL::h($data['content']);
		}
		?>
					</td>
				</tr>
		...
		if($data['public']==0){
			?>
						...
						※下書き以外を選択すると，あとから編集できません。<?php
			ALL::remove_alert();
		}else{
			echo $public[$data['public']];
		}
		?>
					</td>
				</tr>
		...
		if($data['public']==0){
			?>
		<button type="submit" name="btn" value="<?=SQL::encrypt_ks($secret_text.'_edit');?>" onClick="alert_cancel();">
			編集
		</button><?php
		}else{
